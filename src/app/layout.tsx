...
export default function Home() {
  ...
  return (
    <div className="relative min-h-screen bg-[#fff0f6] flex flex-col items-center p-6 overflow-hidden font-[\'Comic Neue\']">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
          <div className="bg-white rounded-2xl shadow-lg p-6 max-w-md text-center border-4 border-pink-200">
            ...
            <p className="text-pink-800 mb-4 text-sm leading-relaxed">
              ...
              <br /><br />Your love drama is safe here. Let it out. We won’t judge.
      ...
      <motion.h1 className="text-4xl font-bold text-pink-700 mb-4 z-10" initial={{ y: -30, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ type: 'spring', stiffness: 100 }}>
      ...
      <div className="w-full max-w-lg bg-white shadow-lg rounded-2xl p-4 z-10">
        <textarea className="w-full p-2 border border-pink-200 rounded mb-4" placeholder="Write your sweet complaint here..." value={newComplaint} onChange={(e) => setNewComplaint(e.target.value)} rows={4} />
        <button onClick={handleSubmitComplaint} disabled={loading} className="bg-pink-500 hover:bg-pink-600 text-white px-4 py-2 rounded w-full flex justify-center items-center">
      ...
      <AnimatePresence>
        {moodMessage && (
          <motion.div className="fixed bottom-4 bg-pink-400 text-white px-6 py-3 rounded-full font-semibold shadow-lg z-50" initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 40 }} transition={{ duration: 0.4 }}>
            {moodMessage}
          </motion.div>
        )}
      </AnimatePresence>

      <motion.button onClick={saySomethingCute} className="mt-6 bg-pink-400 hover:bg-pink-500 text-white font-bold py-2 px-6 rounded z-10" whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}>
        💖 Say Something Cute
      </motion.button>

      {/* Secret Section Toggle */}
      <div className="mt-10 text-center z-10">
        <button onClick={() => setShowSecret(!showSecret)} className="text-sm text-pink-600 underline hover:text-pink-800 flex items-center justify-center mx-auto">
          <Lock className="h-4 w-4 mr-1" /> Reya Only Section
        </button>
        <AnimatePresence>
          {showSecret && (
            <motion.div className="mt-4 p-4 bg-pink-100 border border-pink-300 rounded-xl text-pink-800 max-w-md mx-auto" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.3 }}>
              💌 Hi Reya, you're my most precious human. This section is just for you.
              <br />Thank you for being YOU. You're perfect, beautiful and the sunshine of my every day. 💖
            </motion.div>
          )}
        </AnimatePresence>
      </div>
